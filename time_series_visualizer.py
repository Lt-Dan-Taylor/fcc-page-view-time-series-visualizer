...
# Clean data
df = df.loc[df.value.between(df.value.quantile(0.025),
...
def draw_line_plot():
  ...
  ax.plot(df.index, df.value, linewidth=3, color='purple')
...
def draw_box_plot():
  ...
  boxplot_settings = {
      'flierprops': dict(markerfacecolor='black', marker='d', markersize=4)
  }
